...
public class ProductIndexBuidServiceImpl extends AbstractIndexBuildServiceImpl
{
    ...
    private SolrInputDocument getDoc(Product vo)
    {
        ...
        doc.addField("productName_proN", vo.getProductName());
        if (vo.getBrand() != null)
       ...
       //TODO 临时把库存放入boost中
        ...
        for (Integer storeId : storeIdList) {
        	...
        	doc.addField(storeId+"_salesCount_i", productStat.getBuyCount());	//购买数
        	doc.addField(storeId+"_favoriteCount_i", productStat.getFavoriteCount()); //关注或收藏数
        	doc.addField(storeId + "_reviewCount_i", productStat.getReviewCount());	//评论数
		}
